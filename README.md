...
本文档提供完整的飞书智能任务管理机器人部署指南，从0到1快速部署一个可用的系统。专为组织内部管理外部兼职人员和小任务而设计。Taskbot-MVP 是专为组织管理外部兼职人员和临时小任务而设计的智能任务协作平台。通过飞书群聊与自动化 Bot，实现任务创建、智能匹配、指派、进度提醒、自动验收及统计归档，全流程提效，减少沟通成本。

## 核心功能

- **任务快速创建**：一句指令即可创建新任务，Bot 自动追问任务描述、验收标准、技能要求和截止时间。
- **智能候选人匹配**：Bot 利用大语言模型，从候选人库自动筛选 Top-2 合适人选，生成理由卡片供 HR 指派。
- **一键指派与子群管理**：HR 一键指派后，Bot 自动建群并邀请承接人，任务全程在专属子群沟通和跟进。
- **简历自动解析**：支持上传 PDF/Word 简历，Bot 自动解析为结构化信息，维护候选人库。
- **进度自动提醒**：任务周期过半后自动提醒承接人和 HR，防止遗漏与拖延。
- **自动验收与归档**：承接人完成后用 `/done` 提交，Bot 自动验收（支持代码和非代码任务），并在任务完成 7 天后自动归档群聊。
- **KPI 与任务统计**：自动聚合多维表数据，支持日报、KPI 和任务过程统计。
- **任务和候选人信息维护**：支持多维表和指令方式增删改查，Bot 保证数据同步和实时状态更新。
